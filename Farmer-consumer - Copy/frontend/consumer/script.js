let user, openOrders, deliveredItems, searchInput, dropdown, items, currentElement;
...
window.addEventListener("load", async () => {
    ...
    deliveredItems = get("#delivered-items > tbody");
    loadOrders();
...
async function loadOrders() {
    let data = await sendApiRequest("/consumer/orders")
        .then(assertOK)
        .then((e) => e.json());

    for (let o of data.orders) addOrderRow(o);
}
...
function addOrderRow(p) {
    ...
    } else deliveredItems.appendChild(tr);
...
async function orderComplete(p, element) {
    ...
    await sendApiRequest("/consumer/orders/complete", {
        method: "PATCH",
        body: {
            _id: p._id,
        },
    ...
    order.remove();
    element.remove();
    deliveredItems.appendChild(order);
}

async function loadProducts() {
    let data = await sendApiRequest("/consumer/wholesalers")
        .then(assertOK)
        .then((e) => e.json());

    let item;
    for (let wholesaler of data.wholesalers)
        for (let i of wholesaler.products) {
            item = createDropdownElement(i, dropdown);
            item.wholesaler = wholesaler;
            items.push(item);
        }
}
...
function placeOrder(element) {
    if (!element || !element.wholesaler) return;
...
async function postOrder() {
    ...
    let data = {
        consumerId: user.id,
        wholesalerId: currentElement.wholesaler.id,
        date: get("#Date").value,
        product: currentElement.data,
        quantity: +get("#Quantity").value,
    };

    let order = await sendApiRequest("/consumer/orders", {
        method: "POST",
        body: data,
    })
        .then(assertOK)
...
function updateSearchResults(e) {
    ...
    if (value.length > 0) dropdown.classList.add("show");
    else {
    ...
    for (let element of items)
        if (n >= MAX_HITS) element.hide();
        else if (element.filter(value)) n++;
    ...
    for (let element of items) dropdown.appendChild(element);
}
